...
MIN_VALUE = -MAX_VALUE
MAX_NODE_COUNT = 15
TARGET_DIR = os.path.join(os.path.dirname(__file__), "alpha-beta")
node_cnt = 0
...
def generate_tree():
    ...
    while i < node_cnt:
        node = nodes[i]
        i += 1
        node.max = node.min = random.randint(MIN_VALUE+1, MAX_VALUE-1)  # 这个值表示对我的好处有多大
    return root
...
def tree_to_dot(root, filename, active_node):
    ...
    def dfs(node, parent, depth):
        ...
        g.add_node(pydot.Node(node.id, color=color,fontcolor=fontcolor, label=str(node), shape=shape))
        if parent:
